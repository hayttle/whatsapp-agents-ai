-- =====================================================
-- MIGRAÇÃO: Sistema de Assinaturas com Pacotes
-- =====================================================
-- Este script implementa a nova estrutura DER para gestão
-- de assinaturas com múltiplos pacotes por empresa
...
-- Adicionar novas colunas à tabela subscriptions (se não existirem)
DO $$ 
BEGIN
...
-- Tornar colunas obrigatórias para novos registros (se ainda não forem)
DO $$ 
BEGIN
    -- Verificar se plan_type já é NOT NULL
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
    ...
    -- Verificar se allowed_instances já é NOT NULL
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'subscriptions' 
    ...
    -- Verificar se price já é NOT NULL
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'subscriptions' 
...
-- =====================================================
-- 3. ATUALIZAR TABELA WHATSAPP_INSTANCES
-- =====================================================

-- Adicionar constraint para verificar limite de instâncias
-- (será implementada via trigger)

-- =====================================================
-- 4. CRIAR FUNÇÃO PARA CALCULAR ALLOWED_INSTANCES
-- =====================================================
...
-- =====================================================
-- 5. CRIAR TRIGGER PARA ATUALIZAR ALLOWED_INSTANCES
-- =====================================================
...
-- =====================================================
-- 6. CRIAR FUNÇÃO PARA VERIFICAR LIMITE DE INSTÂNCIAS
-- =====================================================
...
-- Criar trigger para verificar limite ao criar instância (se não existir)
...
COMMENT ON TABLE subscription_payments IS 'Histórico detalhado de pagamentos por assinatura';
COMMENT ON COLUMN subscriptions.plan_type IS 'Tipo do plano: starter, professional, enterprise, custom';
COMMENT ON COLUMN subscriptions.allowed_instances IS 'Número de instâncias permitidas baseado no plano e quantidade';
COMMENT ON COLUMN subscriptions.price IS 'Preço por pacote do plano';

COMMENT ON FUNCTION calculate_allowed_instances IS 'Calcula instâncias permitidas baseado no tipo de plano e quantidade';
COMMENT ON FUNCTION get_tenant_usage_stats IS 'Retorna estatísticas de uso de um tenant específico';
COMMENT ON FUNCTION register_subscription_payment IS 'Registra um pagamento e atualiza status da assinatura se necessário';
